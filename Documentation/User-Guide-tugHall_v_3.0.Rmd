...
6. **Birth_time** - the birthday time step,  e.g., 0, 5.
...
24. **onco_genes** - the binary numbers indicate the driver mutation at the gene related to order of genes in onco as well as order of the next columns with genes' names.
...
```
28. **onco_ID** - the index of the data related to onco object at simulation that has info about lengths of RNA and CDS for each chromosome.
...
33-36. **RNA_(gene's name)**, for example **RNA_APC** - the length of RNA for each gene in the order of names of genes for ONLY FIRST chromosome of a clone.
...
Please, pay attention that probability of mutations depend on length of CDS and RNA of all chromosome but the table has information only for first chromosome. 
...
9. **mut_order** - indicator of mutation order in the simulation, it's used to detect order of mutations in the clone.
...
$m_{CNA} = m_{0,CNA} \times l_{RNA}$, where $l_{RNA}$ is the total region size of all genes of interest which consists of exons as well as introns. 
