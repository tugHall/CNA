...
```{r, echo=FALSE, results='asis'}
x <- read.csv(file = "../Documentation/Example/geneout.txt",header = FALSE, sep = "\t", col.names = c("Gene_name","Hallmark_name", "Weight", "Suppressor_or_oncogene"))
x[is.na(x)] <- ""
...
```{r, echo=FALSE, results='asis'}
x <- read.csv(file = "../Documentation/Example/cloneout.txt",header = TRUE, sep = "\t", nrows = 10)
x <- x[,c(1:16)] 
# library(stringr)
# library(tidyr)
x[is.na(x)] <- ""
...
```{r, echo=FALSE, results='asis'}
x <- read.csv(file = "../Documentation/Example/cloneout.txt",header = TRUE, sep = "\t", nrows = 18)
x[is.na(x)] <- ""
x <- x[,c(1,3,17:27)] 
library(stringr)
...
knitr::kable(x, align = "c", caption = "**Continuation of Table 5. ** Columns are from 28 to end.")
```
